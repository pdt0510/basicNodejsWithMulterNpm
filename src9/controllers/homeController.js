...
const routes = {
  ...
  delUserUrl: '/delete/user', //7ms39ss
  editUserUrl: '/edit/user', //18ms28ss
  updateUserUrl: '/update/user', //28ms40ss
};
...
const delUserPage = async (req, res) => {
  const id = +req.params.id; //11ms23ss, converting to number
  const { table, idCol } = sqlManager;
  const { homeUrl } = routes;
  ...
  return res.redirect(homeUrl);
  // return res.send(`delUserPage - userId: ${+id}`);
};
